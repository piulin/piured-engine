...
class StepQueue extends GameObject {
  ...
  autoplayIt(delta) {
    ...
    const difference = stepTime - currentAudioTime;
    if (difference < 0) {
      const stepList = this.getTopMostStepsList();
      //   console.log(stepList);

      stepList.forEach(step => {
        this.stepPressed(step.kind, step.padId);
      });
    }
    this.updateActiveHolds(currentAudioTime, delta, currentBeat);
    this.updateStepQueue(currentAudioTime);
  ...
  getStepTimeStampFromTopMostStepInfo() {
    if (this.stepQueue.length > 0) return this.stepQueue[0].timeStamp;
    else -100000;
  }
  ...
  getStepFromTopMostStepInfo(index) {
    if (this.stepQueue.length > 0) return this.stepQueue[0].stepList[index];
    else undefined;
  }
  ...
  updateActiveHolds(currentAudioTime, delta, beat) {
    ...
    if (validHold !== null) {
      ...
      const tickCounts = this.beatManager.currentTickCount;
      const difference =
        this.activeHolds.judgmentTimeStampEndReference -
        this.activeHolds.cumulatedHoldTime;
  ...
  updateStepQueue(currentAudioTime) {
    if (this.getLength() > 0) {
      ...
      if (difference < 0 && this.checkForNewHolds) {
        ...
        if (
          this.areThereOnlyHoldsInTopMostStepInfo() &&
          this.areHoldsBeingPressed()
        ) {
  ...
  judgeHolds(delta, currentAudioTime, beat, tickCounts) {
    ...
    const numberOfHits = Math.floor(
      this.activeHolds.timeCounterJudgmentHolds / secondsPerKeyCount
    );
    ...
    if (numberOfHits > 0) {
      ...
      const difference = Math.abs(
        this.activeHolds.lastAddedHold.beginTimeStamp - currentAudioTime
      );
      ...
      if (this.areHoldsBeingPressed()) {
        ...
        this.activeHolds.beginningHoldChunk &&
        difference < this.accuracyMargin
      ) {
  ...
  endJudgingHolds(remainingTime, tickCounts) {
    ...
    if (
      this.areHoldsBeingPressed() &&
      this.activeHolds.wasLastKnowHoldPressed
    ) {
      this.needToAnimateReceptorFX(this.activeHolds.asList());
      this.playerStage.judgment.perfect(numberOfHits);
      // console.log('perfect') ;
  ...
  removeHoldsIfProceeds(currentAudioTime) {
    ...
    for (var i = 0; i < listActiveHolds.length; i++) {
      ...
      if (step !== null && currentAudioTime > step.endTimeStamp) {
        ...
        this.activeHolds.judgmentTimeStampEndReference =
          step.endTimeStamp -
          this.activeHolds.firstHoldInHoldRun.beginTimeStamp;
  ...
  stepPressed(kind, padId) {
    ...
    let [stepInfo, step, hitIndex, difference] = this.getFirstStepWithinMargin(
      currentAudioTime,
      kind,
      padId
    );
  ...
  areStepsInNoteListPressed(noteList) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      if (note.pressed === false) {
        if (
          note instanceof StepHold &&
          this.keyInput.isHeld(note.kind, note.padId)
        ) {
          continue;
        }
        return false;
