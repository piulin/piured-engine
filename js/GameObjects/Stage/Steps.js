...
class Steps extends GameObject {
  ...
  constructSteps() {
    ...
    if (
      this._song.getLevelStyle(this._level) === 'pump-single' ||
      this._song.getLevelStyle(this._level) === 'pump-double'
    ) {
  ...
  composePad(stepDataOffset, padId) {
    ...
    for (var i = 0; i < noteData.length; i++) {
      ...
      for (var j = 0; j < measure.length; j++) {
        ...
        const [currentYPosition, currentTimeInSong] =
          this.beatManager.getYShiftAndCurrentTimeInSongAtBeat(beat);
  ...
  updateCurrentSpeed() {
    this.effectSpeed =
      this.beatManager.getCurrentSpeed() * this._afterSpeedDelta;
  }
    ...
    if (this.lastEffectSpeed !== this.effectSpeed) {
      ...
      this.stepList.forEach(step => {
        if (step instanceof StepNote) {
          ...
          step.stepNote.object.position.y =
            effectSpeed * step.stepNote.object.originalYPos;
  ...
  updateHoldPosition(hold) {
    let distanceToOrigin =
      -this._object.position.y - hold.stepNote.object.position.y;
  ...
  update(delta) {
    ...
    this._object.position.y =
      this.beatManager.currentYDisplacement *
      this._userSpeed *
      this.effectSpeed;
  ...
  set speed(newSpeed) {
    this._afterSpeedDelta = newSpeed / this._userSpeed;
    console.log(this._afterSpeedDelta);
  }
}
