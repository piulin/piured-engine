...
class PlayerStage extends GameObject {
  ...
  configureTouchInputPlayerStage(inputConfig) {
    ...
    if (
      ...
      this._song.getLevelStyle(this._level) === 'pump-halfdouble'
    ) {
  ...
  constructJudgment() {
    this.judgment = new JudgmentScale(
      this._resourceManager,
      this.engine,
      this.accuracyMargin,
      this._song.getLevelStyle(this._level),
      this._song.getLevelDifficulty(this._level),
      this._lifeBar,
      this.frameLog
    );
